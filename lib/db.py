class OpenCartDB:
    ...
    def update_customer(self, id_customer, update_data: dict):
        with self.connection.cursor() as cursor:
    ...
    def select_all_from_table(self, field, value):
        with self.connection.cursor() as cursor:
    ...
    def select_one_from_table(self, field, value):
        with self.connection.cursor() as cursor:
            sql = f"SELECT * FROM {self.table} WHERE {field}={value}"
            cursor.execute(sql)
            selected_data = cursor.fetchall()
    ...
    def select_last_record(self, field):
        with self.connection.cursor() as cursor:
    ...
    def print_select_all_from_table(self):
        with self.connection.cursor() as cursor:
